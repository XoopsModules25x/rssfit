...
function xoops_module_install_rss(\XoopsModule $xoopsMod)
{
    ...
    $sql[] = 'INSERT INTO `'
             . $xoopsDB->prefix($helper->getDirname() . '_misc') . '` VALUES (1, '
    ...
    $sql[] = 'INSERT INTO '
             . $xoopsDB->prefix($helper->getDirname() . '_misc') . ' VALUES '
 ...
 * @return bool
 */
function xoops_module_update_rss(\XoopsModule $xoopsMod, $oldversion)
{
    global $xoopsDB, $xoopsConfig;
    rssfInstallLangFile($xoopsMod, $xoopsConfig['language']);
    ...
    if (!$rows) {
        ...
        $sql[] = 'UPDATE `'
                 . $xoopsDB->prefix($helper->getDirname() . '_misc') . '` SET misc_setting = '
                 . $xoopsDB->quoteString(serialize($intro_setting)) . " WHERE misc_category = 'intro'";
        $sql[] = 'ALTER TABLE `'
                 . $xoopsDB->prefix($helper->getDirname() . '_plugins') . "` ADD `subfeed` TINYINT( 1 ) DEFAULT '0' NOT NULL, ADD `sub_entries` VARCHAR( 2 ) NOT NULL, ADD `sub_link` VARCHAR( 255 ) NOT NULL, ADD `sub_title` VARCHAR( 255 ) NOT NULL, ADD `sub_desc` VARCHAR( 255 ) NOT NULL, ADD `img_url` VARCHAR( 255 ) NOT NULL, ADD `img_link` VARCHAR( 255 ) NOT NULL, ADD `img_title` VARCHAR( 255 ) NOT NULL;";
...
function rssfInsertChannel(&$xoopsMod)
{
    ...
    list($copyright) = $xoopsDB->fetchRow($xoopsDB->query('SELECT conf_value FROM '
                                                          . $xoopsDB->prefix('config') . " WHERE conf_name = 'meta_copyright' AND conf_modid = 1 AND conf_catid = " . XOOPS_CONF_METAFOOTER));
    return 'INSERT INTO ' . $xoopsDB->prefix($helper->getDirname() . '_misc')
